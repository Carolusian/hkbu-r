...
df_addr <- read.csv("addresses_cn_final.csv")
df_addr <- df_addr[df_addr$province!="", ]
df_addr <- count(df_addr, "province")
df_addr$prov_en <- df_addr$province


map2df <- cnmapdf %>%
    plyr::join(df_addr, by = "prov_en") %>%
